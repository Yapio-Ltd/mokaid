...
from release import HERE, check_version, require, required_env, run, validate_assets
...
def metal_options() -> list[str]:
    ...
    component = json.loads(
        run("xcodebuild", "-showComponent", "MetalToolchain", "-json", capture=True)
    )
    identifier = component.get("toolchainIdentifier", "")
    require(
        component.get("status") == "installed"
        and identifier.startswith("com.apple.dt.toolchain.Metal."),
...
def configure(args: argparse.Namespace) -> None:
    ...
    run(
        ...
        "-s",
        "compiler.cppstd=20",
        # The checked-in presets use Ninja on both platforms. Without this,
        # Conan assumes Visual Studio for MSVC and generates a different layout.
        "-c",
        "tools.cmake.cmaketoolchain:generator=Ninja",
        "--build=missing",
    )
    toolchain = DESKTOP / "build/conan/build/Release/generators/conan_toolchain.cmake"
    require(
        toolchain.is_file(),
        "Conan did not generate the Release Ninja toolchain at the expected path",
    )
    ...
    run(
        ...
        "--preset",
        preset,
        f"-DMOKAID_VERSION={numeric}",
        f"-DMOKAID_RELEASE_VERSION={version}",
        f"-DPython3_EXECUTABLE={sys.executable}",
        f"-DMOKAID_RELEASE_CHANNEL={channel}",
        f"-DMOKAID_BETA={'ON' if channel == 'beta' else 'OFF'}",
        "-DMOKAID_ENABLE_UPDATES=ON",
        f"-DMOKAID_UPDATE_PUBLIC_KEY={key}",
        f"-DMOKAID_UPDATER_SDK={sdk}",
    ...
    require(
        any(
            test.get("name") == "desktop.real_assets" for test in tests.get("tests", [])
        ),
...
def sbom() -> None:
    ...
    files = [
        {
            "fileName": "./" + p.relative_to(stage).as_posix(),
            "SPDXID": "SPDXRef-File-"
            + hashlib.sha256(p.relative_to(stage).as_posix().encode()).hexdigest()[:24],
    ...
    document = {
        ...
        "creationInfo": {
            ...
            "creators": ["Tool: mokaid-desktop-ci"],
        },
        "files": files,
        "packages": [],
