    ...
    static const auto catalog = [] {
        ...
        QList<FeatureDescriptor> result{
            ...
            page("marketplace","Marketplace","marketplace","/api/marketplace/listings","",{
                ...
                action("publish","List agent","POST","/api/marketplace/listings",false,true,
                    {field("agent_id","Agent ID","text",true),
                     field("mode","Mode","enum",true,{"sale","rent"}),
                     field("rent_billing","Rent billing","enum",false,{"subscription","fixed"}),
                     field("fixed_days","Fixed days","enum",false,{"7","30","90"}),
                     field("price_cents","Price in cents","int",true),
                     field("title","Title"),field("description","Description","multiline")}),
                action("pause","Pause listing","POST","/api/marketplace/listings/{id}/pause",true,true),
                action("resume","Resume listing","POST","/api/marketplace/listings/{id}/resume",true,true),
                action("connect-onboard","Set up payouts","POST","/api/marketplace/connect/onboard",false,true,
