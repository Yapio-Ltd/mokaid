...
functional parity with every web screen. The registry has 16 client descriptors
...
The shared midnight/violet shell uses dedicated Agents and Office views. Other
pages use task boards, project/file cards, a month calendar with an agenda,
mail list/detail, real analytics metrics, and grouped workspace/account settings.
The Knowledge destination is intentionally absent from native navigation,
registration and global-search results; stored knowledge remains unchanged.
See [the native design contract](../../DESIGN.md) for visual and data rules.
...
`visibleRecords`, `allRecords`, `overview`, and `selectedRecord` expose bounded,
recursively credential-filtered presentation data, including real nested skills
and related records. `_rowId` supplies stable identity for presentation arrays.
Search notifies these properties; report navigation preserves selected identity.
...
- Nested mail rules/accounts, calendar metadata and other
...
- Drag-and-drop calendar/board editing and advanced analytics exploration. The shell's onboarding controller
...
- Browser-layout parity, all copy, accessibility behavior and end-to-end human
  acceptance on the 31 registered screens still need dedicated verification.
...
`desktop-native-pages-qml` renders the dedicated Agents view and eight primary
feature views using real FeatureController instances and isolated HTTP fixtures.
It checks actual-record selection, filtering, list/grid changes, the selected
agent mission entry point, inspector tabs, missing metrics, and empty rosters.
It captures wide and minimum content sizes and rejects QML runtime warnings.
Office shell geometry/retention was also checked with a stubbed renderer; this
does not replace live native-viewport validation.
