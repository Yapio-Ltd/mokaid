...
inline bool allowedLoopbackRequest(const QByteArray& request, const QUrl& callback, const QString& state) {
    ...
    if (start.size() != 3 || start[0] != "GET" || start[2] != "HTTP/1.1") return false;
    ...
    const QUrl target(QString::fromLatin1(start[1]));
    const auto items = QUrlQuery(target).queryItems(QUrl::FullyDecoded);
    if (!target.isRelative() || target.hasFragment() || target.path() != "/callback" || items.size() != 2) return false;
