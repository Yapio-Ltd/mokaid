...
import { useMicrosoftOauthCallback } from "@/api/hooks";
...
export function MicrosoftCallbackPage() {
  ...
  const callback = useMicrosoftOauthCallback();
  const mutateAsyncRef = useRef(callback.mutateAsync);
  mutateAsyncRef.current = callback.mutateAsync;

  const [status, setStatus] = useState<Status>("working");
  const [message, setMessage] = useState("Finalizing the Microsoft connection…");
  ...
  useEffect(() => {
    ...
    async function finalize() {
      ...
      if (!token) {
        setStatus("error");
        setMessage("You must be signed in to complete the Microsoft connection.");
        return;
      }
      ...
      if (!code || !state) {
        setStatus("error");
        setMessage(
          params.get("error_description") ?? "Microsoft did not return an authorization code.",
        );
        return;
      }

      const dedupeKey = oauthDedupeKey("microsoft", code);
      if (sessionStorage.getItem(dedupeKey) === "done") {
        if (notifyOauthOpener("microsoft")) {
          window.close();
        } else {
  ...
  return (
    <div className="flex h-full flex-col items-center justify-center gap-6 bg-bg-deep px-6">
      ...
      <div className="mk-neon-panel flex w-full max-w-sm flex-col items-center gap-4 p-8 text-center">
        ...
        <div>
          <h1 className="text-sm font-bold text-text">Microsoft connection</h1>
