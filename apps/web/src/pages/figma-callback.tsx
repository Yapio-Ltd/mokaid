...
import { useFigmaOauthCallback } from "@/api/hooks";
...
export function FigmaCallbackPage() {
  const navigate = useNavigate();
  const callback = useFigmaOauthCallback();
  const token = useAuthStore((s) => s.token);
  const [status, setStatus] = useState<Status>("working");
  const [message, setMessage] = useState("Finalizing the Figma connection…");
  ...
  useEffect(() => {
    ...
    if (!token) {
      setStatus("error");
      setMessage("You must be signed in to complete the Figma connection.");
      return;
    }

    if (!code || !state) {
      setStatus("error");
      setMessage(params.get("error_description") ?? "Figma did not return an authorization code.");
      return;
    }
    ...
    callback.mutate(
      {
        code,
        state,
        redirect_uri: `${window.location.origin}/oauth/figma/callback`,
      },
      {
        onSuccess: (result) => {
          setStatus("success");
          setMessage(
            result.data.connected_account
              ? `Connected as ${result.data.connected_account}. Redirecting to the MCP Hub…`
              : "Figma is connected. Redirecting to the MCP Hub…",
          );
  ...
  return (
    <div className="flex h-full flex-col items-center justify-center gap-6 bg-bg-deep px-6">
      ...
      <div className="mk-card-raised flex w-full max-w-sm flex-col items-center gap-4 p-8 text-center">
        ...
        <div>
          <h1 className="text-sm font-bold text-text">Figma connection</h1>
