...
vi.mock("@/components/marketing/site-header", () => ({
  SiteHeader: () => <header>Site navigation</header>,
}));
...
const release = {
  ...
  downloads: {
    "macos-arm64": {
      url: "https://downloads.mokaid.com/releases/1.2.3/Mokaid-1.2.3-macos-arm64.dmg",
      sha256: "a".repeat(64),
      size: 100_000,
      minimumOS: "13.0",
    },
    "windows-x64": {
      url: "https://downloads.mokaid.com/releases/1.2.3/Mokaid-1.2.3-windows-x64.exe",
      sha256: "b".repeat(64),
      size: 200_000,
      minimumOS: "10.0.22000",
    },
  },
};
...
describe("desktop release downloads", () => {
  ...
  it("offers only the two verified versioned artifact links", async () => {
    const fetchRelease = vi
      .fn()
      .mockResolvedValue({ status: 200, ok: true, json: async () => release });
    vi.stubGlobal("fetch", fetchRelease);
    render(<DownloadPage />);
    expect(await screen.findByRole("link", { name: "Download for macOS" })).toHaveAttribute(
      "href",
      release.downloads["macos-arm64"].url,
    );
    expect(screen.getByRole("link", { name: "Download for Windows" })).toHaveAttribute(
      "href",
      release.downloads["windows-x64"].url,
    );
    expect(screen.getByText("Apple Silicon · 13.0")).toBeInTheDocument();
    expect(screen.getByText("64-bit Intel / AMD · 10.0.22000")).toBeInTheDocument();
    expect(screen.getByRole("link", { name: "Manage my account" })).toHaveAttribute(
      "href",
      "/account",
    );
    expect(fetchRelease).toHaveBeenCalledWith(
      "https://downloads.mokaid.com/stable/release.json",
    ...
    const untrusted = {
      ...
      downloads: {
        ...
        "macos-arm64": {
          ...
          url: "https://mokaid-downloads.example/installer.dmg",
        },
      },
    };
  ...
  it("allows retry and does not retain stale download links after failure", async () => {
    const fetchRelease = vi
      ...
      .mockResolvedValueOnce({ status: 200, ok: true, json: async () => release });
    vi.stubGlobal("fetch", fetchRelease);
    render(<DownloadPage />);
