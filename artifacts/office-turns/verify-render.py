"""Validate the final turning/robe/camera revision through native Metal."""
import hashlib
import json
import os
...
report = {
    ...
    "note": "Debug offscreen Metal API Validation on this machine; not full-app FPS.",
    "cases": [],
}
for name, width, height, seconds, status in [
    ("final-desk", 1807, 835, 11, "working"),
    ("final-chat", 1110, 835, 71, "idle"),
    ("final-social", 1807, 835, 100, "idle"),
]:
    ...
    row = dict(name=name, viewport=[width, height], simulationSeconds=seconds,
               status=status, gpu=geometry[1], drawCalls=int(geometry[2]),
               triangles=int(geometry[3]), cpuEncodeMeanMs=float(stats[1]),
               gpuMeanMs=float(stats[2]), passed=True)
    report["cases"].append(row)
    print(json.dumps(row), flush=True)
(OUT / "native-render-validation.json").write_text(json.dumps(report, indent=2) + "\n")
